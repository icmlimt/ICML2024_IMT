...
namespace storm {
    namespace storage {
        namespace sparse {
            ...
            void ChoiceOrigins::removeRows(storm::storage::BitVector const& rows) {
                size_t rowIter = 0;
                for (auto row = indexToIdentifier.begin(); row != indexToIdentifier.end(); ++rowIter) {
                    row = rows[rowIter] ? indexToIdentifier.erase(row) : ++row;
                }
            }
        }
    }
