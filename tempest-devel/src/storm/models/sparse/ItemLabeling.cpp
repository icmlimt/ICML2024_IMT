...
namespace storm {
    namespace models {
        namespace sparse {
            ...
            void ItemLabeling::removeItems(storage::BitVector const& items) {
                STORM_LOG_THROW(items.size() <= this->itemCount, storm::exceptions::InvalidArgumentException, "Cannot remove " << items.size() << " items from labeling with size " << itemCount << ".");
                this->itemCount = itemCount - items.getNumberOfSetBits();
                STORM_LOG_DEBUG("new itemcount: " << this->itemCount);
                std::vector<storage::BitVector> newLabelings;
                for(auto &labeling : labelings){
                    storage::BitVector newLabeling(this->itemCount, false);
                    size_t newIndex = 0;
                    for(size_t oldIndex = 0; oldIndex < labeling.size(); ++oldIndex){
                        if(!items[oldIndex]){
                            newLabeling.set(newIndex, labeling[oldIndex]);
                            ++newIndex;
                        }
                    }
                    newLabelings.push_back(newLabeling);
                }
                this->labelings = newLabelings;
            }

            void ItemLabeling::removeItems(storage::BitVector&& items) {
                STORM_LOG_THROW(items.size() <= this->itemCount, storm::exceptions::InvalidArgumentException, "Cannot remove " << items.size() << " items from labeling with size " << itemCount << ".");
                this->itemCount = itemCount - items.getNumberOfSetBits();
                STORM_LOG_DEBUG("new itemcount: " << this->itemCount);
                std::vector<storage::BitVector> newLabelings;
